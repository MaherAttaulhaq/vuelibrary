  ...
  <section class="bg-muted pb-16 md:pb-24">
    <div class="mx-auto max-w-container px-4 md:px-8">
      <div class="flex flex-col gap-8">
        <p class="text-center text-md font-medium text-muted-500 dark:text-muted-400">
          Join 4,000+ companies already growing
        </p>
        <div class="flex flex-wrap justify-center gap-x-8 gap-y-4 xl:gap-x-6">
          <img v-for="company in companies" :key="company.name" :alt="company.name" :src="company.color" class="h-9 md:h-10 dark:hidden">
          <img v-for="company in companies" :key="`${company.name}-dark`" :alt="company.name" :src="company.white" class="h-9 opacity-85 not-dark:hidden md:h-10">
        </div>
      </div>
    </div>
  </section>
