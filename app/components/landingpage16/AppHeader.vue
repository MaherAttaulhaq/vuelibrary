...
<template>
  <header class="relative flex h-16 w-full items-center justify-center md:h-18">
    <div class="flex size-full max-w-container flex-1 items-center pr-3 pl-4 md:px-8">
      <div class="flex w-full justify-between gap-4">
        <div class="flex flex-1 items-center gap-5">
          <NuxtLink to="/" class="hidden h-7 w-max items-center justify-start overflow-visible md:flex">
            <svg viewBox="0 0 38 38" fill="none" class="size-8 origin-center scale-[1.2]">
              <path
            ...
            <span class="ml-3 text-lg font-semibold text-primary">
              Untitled UI
            </span>
          </NuxtLink>
            ...
            <ul class="flex items-center gap-0.5">
              <li v-for="item in navigationItems" :key="item.label">
                <UDropdownMenu v-if="item.type === 'dropdown'" :items="item.items">
                  <UButton variant="link" class="px-1.5 py-1 text-sm font-semibold text-muted-600 hover:text-muted-900 dark:text-muted-400 dark:hover:text-muted-100">
                    {{ item.label }}
                    <Icon name="lucide:chevron-down" class="size-4" />
                  </UButton>
                </UDropdownMenu>
                <ULink v-else :to="item.to" class="flex items-center rounded-lg px-1.5 py-1 text-sm font-semibold text-muted-600 hover:text-muted-900 dark:text-muted-400 dark:hover:text-muted-100">
                  {{ item.label }}
                </ULink>
              </li>
            </ul>
        ...
        <UButton variant="ghost" color="neutral" class="ml-auto rounded-lg p-2 md:hidden" @click="toggle">
    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 bg-muted/95 backdrop-blur-sm md:hidden">
        ...
        <nav class="flex flex-col gap-4">
          <div v-for="item in navigationItems" :key="item.label">
            <UDropdownMenu v-if="item.type === 'dropdown'" :items="item.items" :ui="{ content: 'static w-full' }">
              <UButton variant="link" color="neutral" class="w-full justify-between text-lg font-semibold">
                {{ item.label }}
                <Icon name="lucide:chevron-down" class="size-4" />
              </UButton>
            </UDropdownMenu>
            <ULink v-else :to="item.to" class="py-2 text-lg font-semibold text-muted-600 dark:text-muted-400" @click="close">
              {{ item.label }}
            </ULink>
          </div>
          <div class="mt-4 flex flex-col gap-3 border-t border-muted-200 pt-4 dark:border-muted-800">
            <UButton variant="ghost" color="neutral" block class="justify-center">
              Log in
            </UButton>
            <UButton variant="solid" color="primary" block class="justify-center">
              Sign up
            </UButton>
          </div>
        </nav>
